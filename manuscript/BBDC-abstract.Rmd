...
anova(lm(udbpCrRatio ~ mcr_status + eGFR_status + mcr_status*eGFR_status, data = ds_base))



# Variance-Covariance matrix:

var(PROMISE_data$BMI, na.rm = TRUE)

anthro <- PROMISE_data[c("BMI", "Height", "Weight")]
cov(anthro, anthro, use = "complete.obs")
...
#   polish_transform_estimates(function(x) exp(x)) # "Back-transform" if log
  ...
  add_variables("yvars", "ISI") %>% 
  add_variables("xvars", c("BMI", "Weight")) %>% 
  add_variables("covariates", c("Sex", "Ethnicity")) %>% 
  construct() %>% 
  add_variables("covariates", "VN") %>% 
  construct() %>% 
