...
ds_base %>% 
  ...
  geom_point(colour = "#0db7c4") + #mapping = aes(colour = CRP)
...
ds_mcr %>% 
  ...
  select(mcr_status, UDBP, UrineCreatinine) %>% 
  na.omit() %>% 
  box_plot2("mcr_status", "log(UDBP/UrineCreatinine)", 
            "Albuminuria",
            "log uVDBP:Creatinine") +
...
ds_sub <- ds %>% 
  ...
  # group_by(mcr_status) %>% 
  # mutate(n = ifelse(row_number()==n(), paste("n=", n()), NA))
  # filter(mcr_status == "Macroalbuminuria")

ds_sub %>%
  select(udbpCrRatio, VitaminD, PTH_status) %>% 
  na.omit() %>% 
  scatter_plot("log(udbpCrRatio)", "VitaminD",
               "log UDBP:Creatinine",
               "Serum 25(OH)D (nmol/L)") +
  geom_smooth(method = "lm", colour = "grey50") +
  facet_grid(~PTH_status, switch = "x")
  # geom_text(aes(label = n), y = 40, size = 3, colour = "grey40")
