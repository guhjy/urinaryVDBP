...
ds %>% 
  ...
  scatter_plot("eGFR", "log(UDBP/UrineCreatinine)",
            "Estimated GFR (ml/min/1.73m^2)", "log uVDBP:Creatinine") +
...
ds_visit %>% 
  ...
  theme(axis.title.x = element_blank()) +
  facet_grid(~fVN, switch = "x") +
  geom_text(aes(label = n), y = 5.6 , size = 3, colour = "grey40")
...
interaction.plot(ds_base$mcr_status, ds_base$eGFR_status, ds_base$udbpCrRatio)

mcr_eGFR <- lm(udbpCrRatio ~ mcr_status + eGFR_status + mcr_status*eGFR_status, data = ds_base)
anova(mcr_eGFR)

qqnorm(mcr_eGFR$res)
plot(mcr_eGFR$fitted, mcr_eGFR$res)

ds %>% 
  filter(VN == 1, UDBP < 5000) %>% 
  select(udbpCrRatio, VitaminD, Ethnicity) %>% 
  na.omit() %>% 
  ggplot(aes(x = log(udbpCrRatio), y = VitaminD)) +
  geom_point(mapping = aes(colour = Ethnicity)) 

anova(lm(VitaminD ~ udbpCrRatio + Ethnicity, data = ds_base))
...
ds_visit %>% 
  ...
  geom_text(aes(label = n), y = -7.5, size = 3, colour = "grey40")
